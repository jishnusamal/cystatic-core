...
class GitHubRenderer:
    ...
    def _prepare_render_data(self, ocm: OperationalChangeModel) -> dict[str, Any]:
        ...
        data["change"] = {
            ...
            "removed_symbols": [
                self._render_symbol(s) for s in ocm.change.removed_symbols
            ],
            "modified_symbols": [
                {
    ...
    def _render_behavior(self, behavior: Any) -> dict[str, Any]:
        ...
        return {
            "id": behavior.id,
            "name": behavior.name,
            "type": behavior.kind.value
            if hasattr(behavior.kind, "value")
            else str(behavior.kind),
    ...
    def render_artifact(
        self, artifact: EngineeringDiscoveryArtifact, context: dict[str, Any]
    ) -> str:
        """
    ...
    def _prepare_artifact_data(
        self, artifact: EngineeringDiscoveryArtifact
    ) -> dict[str, Any]:
        """
        ...
        data["change"] = {
            ...
            "added_symbols": [
                self._render_symbol(s) for s in artifact.change.added_symbols
            ],
            "removed_symbols": [
                self._render_symbol(s) for s in artifact.change.removed_symbols
            ],
            "modified_symbols": [
                {
                    "symbol": self._render_symbol(m.symbol),
        ...
        data["execution"] = {
            ...
            "entry_points": [
                self._render_entry_point(ep) for ep in artifact.entry_points
            ],
            "terminal_points": [
                self._render_terminal_point(tp) for tp in artifact.terminal_points
            ],
            "shared_executions": [
                self._render_shared_execution(se) for se in artifact.shared_executions
            ],
        }
    ...
    def render_simple(self, ocm: OperationalChangeModel) -> str:
        ...
        """
        language = (
            ocm.repository.metadata.get("language", "unknown")
            if ocm.repository is not None
            else "unknown"
        )
        files_count = (
            len(ocm.repository.metadata.get("files", {}))
            if ocm.repository is not None
            else 0
        )
        symbols_count = len(ocm.repository.symbols) if ocm.repository is not None else 0
