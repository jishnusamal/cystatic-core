        ...
        for file_fact in facts.files:
            ...
            for sym in facts.symbols:
                if sym.file_id == file_id:
                    ...
                    symbols_entries.append(SymbolEntry(
                        name=sym.name,
                        kind=sym.kind.value,
                        file=file_path,
            ...
            for imp in facts.imports:
                if imp.source_file_id == file_id:
                    names_tuple = tuple(imp.imported_name.split(", ")) if imp.imported_name else ()
                    imports_entries.append(ImportEntry(
                        module=imp.module,
                        names=names_tuple,
            ...
            for call in facts.calls:
                ...
                if caller_fqn and file_path in caller_fqn:
                    ...
                    callee_name = _extract_name_from_fqn(callee_fqn) if callee_fqn else "unknown"

                    calls_entries.append(CallEntry(
                        caller=caller_name,
                        callee=callee_name,
            ...
            for ref in facts.references:
                ...
                if source_fqn and file_path in source_fqn:
                    ...
                    target_name = _extract_name_from_fqn(target_fqn) if target_fqn else "unknown"

                    references_entries.append(RawReference(
                        name=target_name,
                        kind="call",
                        file=file_path,
            ...
            for tr in facts.type_relationships:
                ...
                if source_fqn and file_path in source_fqn:
                    ...
                    type_relationships_entries.append(TypeRelationshipEntry(
                        source=source_name,
                        target=target_name,
            ...
            for ep in facts.endpoints:
                ...
                if handler_fqn and file_path in handler_fqn:
                    ...
                    entrypoints_entries.append(EntrypointEntry(
                        route=route,
            ...
            for db in facts.database_relationships:
                ...
                if sym_fqn and file_path in sym_fqn:
                    ...
                    persistence_entries.append(PersistenceEntry(
                        name=sym_name,
                        kind="table",
                        file=file_path,
                    ))
            ...
            for pub in facts.event_publications:
                ...
                if sym_fqn and file_path in sym_fqn:
                    sym_name = _extract_name_from_fqn(sym_fqn)
                    event_entries.append(EventEntry(
            ...
            for test_rel in facts.test_relationships:
                ...
                if test_fqn and file_path in test_fqn:
                    ...
                    target_name = _extract_name_from_fqn(target_fqn) if target_fqn else ""
                    
                    test_entries.append(TestEntry(
                        name=test_name,
                        kind="function",
                        file=file_path,
