...
class RepositoryFacts:
    ...
    database_relationships: tuple[DatabaseRelationship, ...] = field(
    ...
    _file_map: dict[FileId, File] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    _symbol_map: dict[SymbolId, Symbol] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _calls_from: dict[SymbolId, tuple[Call, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    _calls_to: dict[SymbolId, tuple[Call, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _references_from: dict[SymbolId, tuple[Reference, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    _references_to: dict[SymbolId, tuple[Reference, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _imports_from: dict[FileId, tuple[Import, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _type_relationships_from: dict[SymbolId, tuple[TypeRelationship, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    _endpoints_for: dict[SymbolId, tuple[Endpoint, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _database_relationships_for: dict[SymbolId, tuple[DatabaseRelationship, ...]] = (
        field(default_factory=dict, init=False, repr=False, compare=False)
    )

    _publications_for: dict[SymbolId, tuple[EventPublication, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    _subscriptions_for: dict[EventId, tuple[EventSubscription, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )

    _tests_for: dict[SymbolId, tuple[TestRelationship, ...]] = field(
        default_factory=dict, init=False, repr=False, compare=False
    )
    ...
    def __post_init__(self):
        ...
        object.__setattr__(
            self, "_calls_from", {k: tuple(v) for k, v in calls_from.items()}
        )
        object.__setattr__(
            self, "_calls_to", {k: tuple(v) for k, v in calls_to.items()}
        )
        ...
        object.__setattr__(
            self, "_references_from", {k: tuple(v) for k, v in refs_from.items()}
        )
        object.__setattr__(
            self, "_references_to", {k: tuple(v) for k, v in refs_to.items()}
        )
        ...
        object.__setattr__(
            self, "_imports_from", {k: tuple(v) for k, v in imports_from.items()}
        )
        ...
        object.__setattr__(
            self, "_type_relationships_from", {k: tuple(v) for k, v in tr_from.items()}
        )
        ...
        object.__setattr__(
            self, "_endpoints_for", {k: tuple(v) for k, v in eps.items()}
        )
        ...
        object.__setattr__(
            self,
            "_database_relationships_for",
            {k: tuple(v) for k, v in db_rels.items()},
        )
        ...
        object.__setattr__(
            self, "_publications_for", {k: tuple(v) for k, v in pub_rels.items()}
        )
        ...
        object.__setattr__(
            self, "_subscriptions_for", {k: tuple(v) for k, v in sub_rels.items()}
        )
        ...
        object.__setattr__(
            self, "_tests_for", {k: tuple(v) for k, v in test_rels.items()}
        )
    ...
    def database_relationships_for(
        self, symbol_id: SymbolId
