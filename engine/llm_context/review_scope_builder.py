...
def classify_symbol(name: str, symbol_id: str, file_path: str, kind: str) -> str:
    ...
    if (
        ...
        or ".gitlab-ci" in path_lower
        or "docker" in path_lower
        or "makefile" in path_lower
        or "setup.py" in path_lower
        or "setup.cfg" in path_lower
        or "pyproject.toml" in path_lower
        or "tox.ini" in path_lower
        or ".gitignore" in path_lower
        or "requirements" in path_lower
        or "uv.lock" in path_lower
    ):
        return "Build"
    ...
    if (
        "dev/" in path_lower
        or "scripts/" in path_lower
        or "cli.py" in path_lower
        or "install" in path_lower
        or "bootstrap" in path_lower
        or ".venv" in path_lower
    ):
        return "Tooling"
    ...
    if (
        ...
        or "fixture" in name_lower
        or "assert" in name_lower
        or kind_lower == "test"
    ):
        is_test = True
    ...
    # 5. Framework
    if (
        first_part in FRAMEWORK_MODULES
        or name in FRAMEWORK_NAMES
        or "middleware" in name_lower
        or "router" in name_lower
        or "route" in name_lower
        or ("app" == name_lower and kind_lower in {"module", "package", "application"})
    ):
        return "Framework"

    # 6. Infrastructure
    if (
        first_part in ORM_MODULES
        ...
        or "redis" in name_lower
        or "logger" in name_lower
        or "logging" in name_lower
    ...
    # 7. Event
    if (
        kind_lower in {"worker", "task", "event"}
        or "event" in name_lower
        or "publish" in name_lower
        or "subscribe" in name_lower
        or "queue" in name_lower
        or "consumer" in name_lower
        or "producer" in name_lower
    ):
        return "Event"

    # 8. API
    if (
        kind_lower in {"endpoint", "route"}
        or "api" in name_lower
    ...
    # 9. Validation
    if (
        "validate" in name_lower
        or "validator" in name_lower
        or "check" in name_lower
        or "schema" in name_lower
    ):
        return "Validation"
    ...
    # 11. Database Schema
    if "model" in name_lower or "table" in name_lower or "entity" in name_lower:
...
def prune_review_context(
    review_context: ReviewContext, settings: CompilerSettings | None = None
) -> ReviewContext:
        ...
        for ep in review_context.execution.entry_points:
            ...
            for step in ep.execution_chain:
                is_changed = step.changed or (
                    step.symbol and step.symbol.id in changed_symbols
                )
    ...
    if review_context.discoveries:
        for d in review_context.discoveries:
            ...
            for ref in d.references:
                if not (
                    is_compiler_metadata(ref.id)
                    or is_compiler_metadata(ref.location)
                    or is_compiler_metadata(ref.compiler_artifact)
                ):
                    ref_key = (ref.id, ref.kind, ref.location, ref.compiler_artifact)
                    pruned_supporting_nodes = tuple(
                        n for n in ref.supporting_nodes if not is_compiler_metadata(n)
                    )
            ...
            for k, v in d.facts.items():
                if not (
                    is_compiler_metadata(k)
                    or (isinstance(v, str) and is_compiler_metadata(v))
                ):
                    pruned_facts[k] = v
    ...
    if review_context.change and review_context.change.files:
        for f in review_context.change.files:
            ...
            for c in f.changes:
                ...
                if (
                    sym_cat not in NOISE_CATEGORIES
                    or is_symbol_in_exec
                    or is_symbol_in_disc
                ):
    ...
    pruned_change = ChangeContext(
        summary=review_context.change.summary
        if review_context.change
        else ChangeContext().summary,
