...
class ImpactEngine:
    ...
    def calculate_impact(
        ...
        queue_up: deque[tuple[str, int]] = deque((sid, 0) for sid in changed_symbol_ids)
        queue_down: deque[tuple[str, int]] = deque((sid, 0) for sid in changed_symbol_ids)
        
        affected_symbols: set[str] = set(changed_symbol_ids)
        ...
        entry_point_map = {ep.handler_id: ep for ep in all_entry_points}
        ...
        while queue_up and len(visited_up) < self.config.max_visited:
            ...
            for call in callers:
                if call.caller_id not in visited_up:
                    queue_up.append((call.caller_id, depth + 1))
        ...
        while queue_down and len(visited_down) < self.config.max_visited:
            ...
            for call in callees:
                if call.callee_id not in visited_down:
                    queue_down.append((call.callee_id, depth + 1))
