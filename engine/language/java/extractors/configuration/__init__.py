...
class JavaConfigurationExtractor(BaseExtractor):
    ...
    def extract(self, tree: list[str], file_path: str) -> list[dict[str, Any]]:
        ...
        for match in re.finditer(r'@Value\s*\(\s*["\']\$\{([^}]+)\}["\']', content):
            ...
            caller_id = self._find_method_for_line(tree, content[:match.start()].count('\n'))
            default_value = ''
        ...
        for match in re.finditer(r'(?:env|environment|env\.getProperty)\s*\(\s*"([^"]+)"', content):
            caller_id = self._find_method_for_line(tree, content[:match.start()].count('\n'))
        ...
        for match in re.finditer(r'@ConfigurationProperties\s*\(\s*prefix\s*=\s*"([^"]+)"', content):
            config_refs.append({
                'symbol_id': '',
                'config_key': match.group(1),
                'kind': 'config_file',
