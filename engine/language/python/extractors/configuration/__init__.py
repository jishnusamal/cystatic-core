...
class PythonConfigurationExtractor(BaseExtractor):
    ...
    def _extract_config_call(self, node: ast.Call, file_path: str) -> dict[str, Any] | None:
    ...
    def _extract_config_subscript(self, node: ast.Subscript, file_path: str) -> dict[str, Any] | None:
    ...
    def _extract_config_attribute(self, node: ast.Attribute, file_path: str) -> dict[str, Any] | None:
        """Extract config reference from an attribute access like settings.DATABASE_URL."""
        # settings.X or config.X
        if isinstance(node.value, ast.Name) and node.value.id in ('settings', 'config', 'conf', 'cfg'):
            attr_name = node.attr
