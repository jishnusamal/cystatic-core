...
class PassInstrumentation:
    ...
    def record_internal_op(
        self, pass_name: str, method_name: str, op_name: str, elapsed: float
    ):
    ...
    def increment_counter(self, pass_name: str, counter_name: str, amount: int = 1):
        ...
        # Also track globally
        self.global_counters[counter_name] = (
            self.global_counters.get(counter_name, 0) + amount
        )

    def increment_method_counter(
        self, pass_name: str, method_name: str, counter_name: str, amount: int = 1
    ):
    ...
    def print_pass_summary(self):
        ...
        for pass_name, stats in sorted(
            self.pass_stats.items(), key=lambda x: x[1].total_time, reverse=True
        ):
            ...
            log(
                f"  Calls: {stats.call_count}, Avg: {avg_time:.2f}ms, Max: {stats.max_time * 1000:.2f}ms"
            )
    ...
    def print_method_summary(self):
        ...
        for pass_name, method_name, stats in all_methods[:20]:  # Top 20
            ...
            log(
                f"{pass_name}.{method_name:<30} {stats.total_time:>8.2f}s  "
        ...
        for pass_name, pass_stats in self.pass_stats.items():
            for method_name, method_stats in pass_stats.method_stats.items():
                ...
                if significant_ops:
                    ...
                    for op_name, op_time, pct in sorted(
                        significant_ops, key=lambda x: x[1], reverse=True
                    ):
    ...
    def print_top_operations(self, n: int = 50):
        ...
        for i, (pass_name_op, method_name_op, total_time, call_count) in enumerate(
            operations[:n], 1
        ):
            if method_name_op:
            ...
            log(
                f"{i:>3}. {name:<60} {total_time:>8.2f}s  "
    ...
    def print_hotspot_analysis(self):
        ...
        if largest_method:
            ...
            if method_stats.call_count > 0:
                log(
                    f"  Average: {method_stats.total_time / method_stats.call_count * 1000:.2f}ms"
                )
        ...
        for pass_name, pass_stats in self.pass_stats.items():
            if "ast.walk" in pass_stats.counters or "ast.walk" in str(
                pass_stats.counters
            ):
        ...
        for pass_name, pass_stats in self.pass_stats.items():
            for method_name, method_stats in pass_stats.method_stats.items():
                if method_stats.call_count > 1000 and method_stats.total_time > 1.0:
                    log(
                        f"  - {pass_name}.{method_name}: {method_stats.call_count} calls, "
    ...
    def print_profile_summary(self):
        ...
        visitor_time = (
            visitor_timings[0]["elapsed"]
            if visitor_timings
            else sum(s.total_time for s in self.pass_stats.values())
        )
        ...
        sorted_methods = sorted(
            merged_methods.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            file_path = kwargs.get(
                "file_path", args[1].path if len(args) > 1 else "unknown"
            )
