...
class MemoryProfiler:
    def __init__(self, analysis_id: str | None = None):
        ...
        if self.enabled:
            ...
            self._monitor_thread = threading.Thread(
                target=self._monitor_rss, daemon=True
            )
    ...
    def log_tracemalloc_difference(self, stage: str, limit: int = 15):
        ...
        try:
            ...
            report = [
                f"[TRACEMALLOC-TOP][analysis={self.analysis_id}] Top allocations at stage: {stage}"
            ]
            for index, stat in enumerate(top_stats[:limit], 1):
                ...
                report.append(
                    f"  {filename}:{frame.lineno} {size_mb:.2f} MB (count={stat.count})"
                )
