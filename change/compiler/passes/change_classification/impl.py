...
class ChangeClassificationPass(ChangeCompilerPass):
    ...
    def _classify_symbol_changes(self, old_symbol: Symbol, new_symbol: Symbol) -> list[Any]:
        ...
        if self._has_body_change(old_symbol, new_symbol):
            ...
            changes.append(FunctionBodyChange(
        ...
        if old_symbol.visibility != new_symbol.visibility:
            changes.append(VisibilityChange(
        ...
        endpoint_change = self._detect_endpoint_annotation_change(old_symbol, new_symbol)
        if endpoint_change:
    ...
    def _detect_signature_change(self, old_symbol: Symbol, new_symbol: Symbol) -> SignatureChange | None:
    ...
    def _detect_decorator_change(self, old_symbol: Symbol, new_symbol: Symbol) -> DecoratorChange | None:
        ...
        if old_decorators != new_decorators:
            return DecoratorChange(
                ...
                new_decorators=new_decorators
            )
        
        return None
    
    def _detect_superclass_change(self, old_symbol: Symbol, new_symbol: Symbol) -> SuperclassChange | None:
        ...
        if old_superclass != new_superclass:
            return SuperclassChange(
                old_superclass=old_superclass,
                new_superclass=new_superclass
            )
        
        return None
    
    def _detect_interface_change(self, old_symbol: Symbol, new_symbol: Symbol) -> InterfaceChange | None:
    ...
    def _detect_changed_imports(self, context: ChangePassContext) -> list[dict[str, Any]]:
        ...
        for import_id in old_imports:
            if import_id in new_imports:
                ...
                if old_import.name != new_import.name or old_import.properties != new_import.properties:
                    changed_imports.append({
                        'file': new_import.file,
                        'old_import': old_import.name,
                        'new_import': new_import.name,
    ...
    def _detect_changed_endpoints(self, context: ChangePassContext) -> list[dict[str, Any]]:
        ...
        for endpoint_id in old_endpoints:
            if endpoint_id in new_endpoints:
                ...
                if (old_data['endpoint'] != new_data['endpoint'] or
                    old_data['method'] != new_data['method']):
                    changed_endpoints.append({
                        'symbol_id': endpoint_id,
