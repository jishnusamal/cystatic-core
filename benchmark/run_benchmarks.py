...
async def run_pr_analysis_direct(pr_url: str):
    ...
    try:
        ...
        if context:
            if context.base_repository_snapshot and hasattr(context.base_repository_snapshot, "files"):
                file_count = len(context.base_repository_snapshot.files)
                source_bytes = sum(len(content) for content in context.base_repository_snapshot.files.values())
            ...
            if context.head_repository_model:
                ...
                if context.head_repository_model.call_graph and context.head_repository_model.call_graph.edges:
                    call_edge_count = len(context.head_repository_model.call_graph.edges)
                if context.head_repository_model.reference_graph and context.head_repository_model.reference_graph.edges:
                    ref_edge_count = len(context.head_repository_model.reference_graph.edges)
...
def generate_markdown_report(results):
    ...
    lines.append("")
    lines.append("> **Scope** – Baseline memory and execution metrics across small, medium, and large repositories.")
    lines.append("")
    
    # Summary table
    lines.append("## Repository Metrics Summary")
    lines.append("")
    lines.append("| Repository | File Count | Source Bytes | Symbol Count | Call Edges | Reference Edges | Duration (s) | Peak RSS (MB) |")
    lines.append("|---|---|---|---|---|---|---|---|")
    ...
    for pr, res in results.items():
        ...
        for stage in ordered_stages:
            if stage in res["checkpoints"]:
                ...
                lines.append(f"| {stage} | {pt['current_rss']:.1f} | {pt['peak_rss']:.1f} |")
        lines.append("")
