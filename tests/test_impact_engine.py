...
from engine.repository.query.types import Call, CallType, SymbolId, EventPublication, EventPublicationType, EventId

class MockRepositoryQuery(RepositoryQuery):
    def get_symbol(self, symbol_id): return None
    def get_file(self, file_id): return None
    def get_calls_from(self, caller_id): return ()
    
    def get_calls_to(self, callee_id):
        # Mock upward traversal: A calls B
        if str(callee_id) == "python://test.py::B":
            return (Call(caller_id=SymbolId("python://test.py::A"), callee_id=SymbolId("python://test.py::B"), call_type=CallType.DIRECT),)
        return ()
        
    def get_callees(self, caller_id):
        # Mock downward traversal: B calls C
        if str(caller_id) == "python://test.py::B":
            return (Call(caller_id=SymbolId("python://test.py::B"), callee_id=SymbolId("python://test.py::C"), call_type=CallType.DIRECT),)
        return ()
        
    def get_callers(self, callee_id):
        return self.get_calls_to(callee_id)
        
    def get_references_to(self, target_id): return ()
    def get_references_from(self, source_id): return ()
    def get_imports_from(self, file_id): return ()
    def get_imports_to(self, file_id): return ()
    def get_implementations(self, symbol_id): return ()
    def get_usages(self, symbol_id): return ()
    def get_endpoints(self, symbol_id): return ()
    def get_database_relationships(self, symbol_id): return ()
    def get_published_events(self, symbol_id):
        if str(symbol_id) == "python://test.py::C":
            return (EventPublication(symbol_id=SymbolId("python://test.py::C"), event_id=EventId("event1"), publication_type=EventPublicationType.EMIT),)
        return ()
    def get_event_consumers(self, event_id): return ()
    def get_tests(self, symbol_id): return ()
    def get_importers(self, symbol_id): return ()
    def get_imports(self, file_id): return ()
    def get_type_dependents(self, symbol_id): return ()
    def get_type_relationships(self, symbol_id): return ()

    
    def get_entry_points(self):
        return (
            EntryPoint(kind=EntryPointKind.REST_ENDPOINT, route="GET /a", handler_id="python://test.py::A"),
        )
