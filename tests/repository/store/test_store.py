...
def test_round_trip_all_facts():
    ...
    # 3. Call
    call_fact = Call(
        caller_id=SymbolId(10), callee_id=SymbolId(20), call_type=CallType.DIRECT
    )
    sink.add_call(call_fact)

    # 4. Reference
    ref_fact = Reference(
        source_id=SymbolId(10),
        target_id=SymbolId(30),
    ...
    # 6. TypeRelationship
    type_fact = TypeRelationship(
        source_id=SymbolId(10),
        target_id=SymbolId(40),
...
def test_query_equivalence():
    ...
    for r in facts.references:
        sink.add_reference(r)
    for i in facts.imports:
        sink.add_import(i)
    for tr in facts.type_relationships:
        sink.add_type_relationship(tr)
    for ep in facts.endpoints:
        sink.add_endpoint(ep)
    for db in facts.database_relationships:
        sink.add_database_relationship(db)
    for pb in facts.event_publications:
        sink.add_event_publication(pb)
    for sb in facts.event_subscriptions:
        sink.add_event_subscription(sb)
    for t in facts.test_relationships:
        sink.add_test_relationship(t)
    sink.flush()
    ...
    for file_id in (FileId(1), FileId(2), FileId(99)):
        ...
        assert set(in_mem_repo.get_imports(file_id)) == set(
            sqlite_store.get_imports(file_id)
        )
        assert set(in_mem_repo.get_importers(file_id)) == set(
            sqlite_store.get_importers(file_id)
        )
    ...
    for sym_id in (SymbolId(1), SymbolId(2), SymbolId(3), SymbolId(4), SymbolId(99)):
        ...
        assert set(in_mem_repo.get_callers(sym_id)) == set(
            sqlite_store.get_callers(sym_id)
        )
        assert set(in_mem_repo.get_callees(sym_id)) == set(
            sqlite_store.get_callees(sym_id)
        )
        assert set(in_mem_repo.get_references_from(sym_id)) == set(
            sqlite_store.get_references_from(sym_id)
        )
        assert set(in_mem_repo.get_references_to(sym_id)) == set(
            sqlite_store.get_references_to(sym_id)
        )
        assert set(in_mem_repo.get_type_relationships(sym_id)) == set(
            sqlite_store.get_type_relationships(sym_id)
        )
        assert set(in_mem_repo.get_type_dependents(sym_id)) == set(
            sqlite_store.get_type_dependents(sym_id)
        )
        assert set(in_mem_repo.get_endpoints(sym_id)) == set(
            sqlite_store.get_endpoints(sym_id)
        )
        assert set(in_mem_repo.get_database_relationships(sym_id)) == set(
            sqlite_store.get_database_relationships(sym_id)
        )
        assert set(in_mem_repo.get_published_events(sym_id)) == set(
            sqlite_store.get_published_events(sym_id)
        )
        assert set(in_mem_repo.get_tests(sym_id)) == set(sqlite_store.get_tests(sym_id))

    for ev_id in (EventId(100), EventId(999)):
        assert set(in_mem_repo.get_event_consumers(ev_id)) == set(
            sqlite_store.get_event_consumers(ev_id)
        )
